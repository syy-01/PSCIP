...
def shap_analysis(model, X_data, feature_names, selector, sample_size=100, save_dir='shap_plots_rf'):
    """使用训练好的随机森林模型进行SHAP分析，条形图仅显示前20个特征"""
    ...
    # 关键修复：处理多输出情况，确保只使用阳性类别的SHAP值
    if isinstance(shap_values, list) and len(shap_values) == 2:  # 二分类场景
        shap_values = shap_values[1]  # 取阳性类（类别1）的SHAP值
        print("检测到二分类SHAP值，已自动转换为阳性类别的SHAP值")
...
def train_and_evaluate_model(X_train, y_train, X_test, protein_names, feature_names, n_splits=100, k_features=38):
    ...
    for train_index, test_index in tqdm(splits, total=n_splits, desc="训练进度"):
        ...
        y_prob = rfc.predict_proba(X_test_selected)[:, 1]

        # 计算指标
        metrics_dict = {
            'AUC': metrics.roc_auc_score(y_test_split, y_prob),
            'Accuracy': rfc.score(X_test_selected, y_test_split),
        ...
        pred_df['In Corona Probability'] = rfc.predict_proba(X_predict_selected)[:, 1]
        pred_df['Round'] = i
...
def main():
    ...
    rfc_final.fit(X_train_oversampled, y_train_oversampled)
    ...
    shap_analysis(
        ...
        selector=selector,
        sample_size=100,
        save_dir='shap_plots_rf'
    )

    # 最终预测
    final_predictions = pd.DataFrame()
    final_predictions['Protein Name'] = protein_names
    final_predictions['In Corona Probability'] = rfc_final.predict_proba(X_test_selected)[:, 1]
    ...
    # 保存结果
    print("保存结果...")
    with pd.ExcelWriter('2021_08_08_predictions_simplified_gt6_with_shap.xlsx') as writer:
